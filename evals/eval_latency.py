...
REPEATS = int(os.getenv("LATENCY_REPEATS", "5"))
WARMUP_RUNS = int(os.getenv("LATENCY_WARMUP_RUNS", "2"))
...
# ============================================================
# End-to-end: invoke() returns the generated answer and retrieval context.
...
def run_stages(pipeline, question):
    ...
    answer = pipeline.generator.generate(question, context=context)["answer"]
...
def run_stages_streaming(pipeline, question):
    ...
    first_token_t = None
    pieces = []
    for piece in pipeline.generator.generate_stream(question, context=context):
...
# ============================================================
def main():
    pipeline = RagPipeline(model=os.getenv("GEMINI_MODEL", "gemini-3.1-flash-lite"))

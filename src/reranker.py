...
class RerankingRetriever:
    ...
    def invoke(self, query):
        # 1. OVER-RETRIEVE: use Chroma's current query API instead of the removed similarity_search
        results = self.store.query(query_texts=[query], n_results=self.fetch_k, include=["documents", "metadatas"])
        documents = results.get("documents", [[]])[0]
        metadatas = results.get("metadatas", [[]])[0]

        candidates = [
            _Doc(page_content=doc, metadata=meta)
